...
.. code-block:: console
   ...
   git clone https://github.com/zephyrproject-rtos/manifest.git
   echo > .west_topdir

After that, and in order for ``ninja`` to be able to invoke ``west`` you have
two options:

- Install the west bootstrapper with ``pip``, this will provide an entry point
- Create a script that sets ``PYTHONPATH`` to
  :file:`<path to zephyrproject>/west/west/src/` and executes ``python`` with
  the main scripts: :file:`<path to zephyrproject>/west/west/src/west/main.py`
